...
describe('conjugation drill generator', () => {
  it('is deterministic for the same seed', () => {
    const a = genConjugationDrill({
      verb: querer, tense: 'pres', person: '2sg', topicId: 't', type: 'mc', seed: 'x',
    })
  ...
  it('mc options are unique, include the answer, and carry strategies on wrong ones', () => {
    for (const person of PERSONS) {
      const ex = genConjugationDrill({
        verb: querer, tense: 'pres', person, topicId: 't', type: 'mc', seed: `s/${person}`,
      })
  ...
  it('stem-change corruptions appear as distractors', () => {
    ...
    for (let i = 0; i < 20; i++) {
      const ex = genConjugationDrill({
        verb: querer, tense: 'pres', person: '1pl', topicId: 't', type: 'mc', seed: `s${i}`,
      })
  ...
  it('cloze marks the grammatical suffix as strict', () => {
    const ex = genConjugationDrill({
      verb: querer, tense: 'pres', person: '2sg', topicId: 't', type: 'cloze', seed: 's',
    })
  ...
  it('assigns the person-cell skill id', () => {
    const ex = genConjugationDrill({
      verb: ser, tense: 'pres', person: '3pl', topicId: 'es-ser-present', type: 'mc', seed: 's',
    })
...
describe('match drill generator', () => {
  it('produces unambiguous pairs', () => {
    const ex = genMatchDrill({ verb: ser, tense: 'pres', topicId: 't', seed: 's' })
